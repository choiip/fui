...
private:
    ...
    struct SignalLink {
        ...
        bool deactivate(const CbFunction& cbf) {
            ...
            for (SignalLink* link = this->next ? this->next : this;
                 link != this; link = link->next)
...
public:
    ...
    /// ProtoSignal destructor releases all resources associated with this
    /// signal.
    ~ProtoSignal() {
        if (callback_ring_) {
    ...
    /// Operator to remove a signal handler through it connection ID, returns if
    /// a handler was removed.
    bool operator-=(size_t connection) {
        return callback_ring_ ? callback_ring_->remove_sibling(connection)
                              : false;
    }
    /// Return True if callback exist
    operator bool() const { return callback_ring_ != NULL; }
 ...
 * callbacks, and optionally a return result collector class type. Signal
 * callbacks can be added with operator+= to a signal and removed with
 ...
 * returned from emit(). Collectors can be implemented to accumulate callback
 ...
 * also safe. The overhead of an unused signal is intentionally kept very low,
 * around the size of a single pointer. Note that the Signal template types is
 * non-copyable.
 */
template <typename SignalSignature,
          class Collector = internal::CollectorDefault<
              typename std::function<SignalSignature>::result_type> >
struct Signal /*final*/ : internal::ProtoSignal<SignalSignature, Collector> {
...
std::function<R(Args...)> slot(Instance& object, R (Class::*method)(Args...)) {
    return
        [&object, method](Args... args) { return (object.*method)(args...); };
}
