...
static const JOCTET dummy_buffer[] = { (JOCTET) 0xFF, (JOCTET) JPEG_EOI };
...
static boolean jpeg_fill_input_buffer( j_decompress_ptr cinfo )
{
  ...
  if( src->next_input_byte != &dummy_buffer[0] ) {
    // Insert a fake EOI marker - as per jpeglib recommendation
    src->next_input_byte = dummy_buffer;
...
void jpegErrorExit (j_common_ptr cinfo) {
    /* cinfo->err actually points to a jpg_error_manager struct */
    struct jpg_error_mgr* myerr = (struct jpg_error_mgr*) cinfo->err;
                    ...
                    unsigned int* numChannels )
{
  ...
  jsm.next_input_byte = (const JOCTET*) srcData;
  jsm.bytes_in_buffer = srcSize;
	jsm.init_source = jpeg_init_source;
	jsm.fill_input_buffer = jpeg_fill_input_buffer;
	jsm.skip_input_data = jpeg_skip_input_data;
	jsm.resync_to_restart = jpeg_resync_to_restart;
	jsm.term_source = jpeg_term_source;
