...
FontDescriptor *findFont(FontDescriptor* request) {
  for (auto&& fd: webFontDescriptors) {
    ...
    if (fd.lang == request->lang && fd.style == request->style) {
      return new FontDescriptor(fd);
    }
  }
  return nullptr; 
}
...
void setupWebFonts(const std::vector<FontDescriptor>& fontDescriptors) {
  webFontDescriptors = fontDescriptors;
}
